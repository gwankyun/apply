...
#  define APPLY_BOOST_GET(z, n, x) BOOST_PP_COMMA_IF(n) boost::get<n>(x)
    ...
    inline typename boost::result_of<F(T0)>::type apply(F f, boost::tuple<T0>& tuple_)
    {
        return f(boost::get<0>(tuple_));
    }

    template<typename F, typename T0>
    inline typename boost::result_of<F(T0)>::type apply(F f, const boost::tuple<T0>& tuple_)
    {
        return f(boost::get<0>(tuple_));
    }
